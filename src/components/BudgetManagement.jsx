...
import {
  PieChart, TrendingUp, AlertCircle, FileText,
...
const SummaryCard = React.memo(({ title, value, subValue, icon: Icon, color }) => (
  <div className="premium-card flex flex-col justify-between h-full">
    <div className="flex justify-between items-start mb-4">
      <div className={`p-3 rounded-[14px] ${color.bg} bg-opacity-10 flex items-center justify-center`}>
        {typeof Icon === 'string' ? (
          <span className={`text-xl font-bold ${color.text}`}>{Icon}</span>
        ) : (
